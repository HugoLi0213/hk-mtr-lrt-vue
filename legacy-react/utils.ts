...
export const triggerShareImg = async (
  ...
  // 將檔名與任何 hkbus.app 相關內容移除，改為 mtrbus.png
  const file = new File([blob], "mtrbus.png", { type: blob.type });
  if (navigator.share) {
...
export const setSeoHeader = ({
  ...
  document
    ...
    ?.setAttribute(
      "href",
      `https://${window.location.host}${window.location.pathname.replace(/\(/g, "%28").replace(/\)/g, "%29")}`
    );
  ...
  document
    ...
    ?.setAttribute(
      "href",
      window.location.origin + window.location.pathname.replace(`/${lang}`, "/en")
    );
  document
    .querySelector('link[rel="alternative"][hreflang="zh-Hant"]')
    ?.setAttribute(
      "href",
      window.location.origin + window.location.pathname.replace(`/${lang}`, "/zh")
    );
  document
  ...
  document
    ...
    ?.setAttribute("content", `${window.location.origin}${window.location.pathname}`);
  document
